...
val DEFAULT_DBPASSWORD = "jmsuser"
	...
	try {
	  ...
	  //val publisher = tsess.createPublisher(topic)
          val subscriber1 = tsess.createDurableSubscriber( topic, "WesternShipping")
          var txtmsgin = subscriber1.receive(1) 
          val t_msg :TextMessage  = txtmsgin.asInstanceOf[TextMessage]
          tsess.commit()
	  println("\ndestination: " + topic + "\nmessage :\n\n" )
	  println("\nmessage was  with ID="+txtmsgin.getJMSMessageID())
	  println("\nmessage was  ="+t_msg.getText())
